...
module.exports = {
    init: (app, db) => {
        ...
        app.post("/v1/admin/ban", db.requireAuth, db.requireMod, async (req, res) => {
            ...
            db.log(`user ${user.userid} has been ${banType} banned by ${req.user.username} (${req.user.userid}) for the reason: ${reason}`);
...
<script type='text/javascript' src='/js/jquery-1.11.1.min.js'></script>
            ...
            for (let i = 0; i < assets.length; i++) {
                ...
                html += `<tr>
                 <td><a target=”_blank” href="https://www.rbx2016.nl/users/${creator.userid}/profile">${creator.username}</a></td>
                 <td><a target=”_blank” href="https://www.rbx2016.nl/library/${asset.id}">${db.censorText(db.filterText4(asset.name))}</a></td>
